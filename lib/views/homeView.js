...
import { getAllLessons } from '../curriculumHelper.js';
...
export function renderHome(app) {
  ...
  const showNotifBanner = NotificationService.isSupported() &&
    ...
    <div class="home-view">
      ${showNotifBanner ? `
        <div class="notif-banner" id="notif-banner">
          <span>🔔 복습 알림을 받으시겠어요?</span>
          <div class="notif-banner-btns">
            <button class="notif-yes-btn" id="notif-yes">받을게요</button>
            <button class="notif-no-btn" id="notif-no">괜찮아요</button>
          </div>
        </div>
      ` : ''}
      <div class="header">
      ...
      <div class="srs-daily-log">
        ...
        <button class="theme-toggle-btn" id="theme-toggle-btn" title="다크모드 전환">🌙</button>
      </div>
  ...
  document.getElementById('daily-quest-btn').addEventListener('click', () => {
    if (hasDueCards || !stampEarned) window.location.hash = '#quiz/srs';
  });
  ...
  for (let i = 0; i < stageLessons.length; i++) {
    ...
    html += `
      <a href="${isUnlocked ? '#lesson/' + lesson.id : '#home'}"
         class="${nodeClass}" title="${lesson.row}"
