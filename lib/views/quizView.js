  ...
  document.querySelectorAll('.quiz-choice-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (isCorrect) {
        ...
        if (!app.wrongCards) app.wrongCards = [];
        app.wrongCards.push(card);
      }
...
export function showQuizResult(app, totalCards) {
  ...
  const wrongCount = (app.wrongCards || []).length;
  const showWrongRetry = wrongCount > 0 && !app.isRetryMode;
  ...
  app.appEl.innerHTML = `
    <div class="lesson-complete-view">
      ...
      <div class="complete-actions">
        ${showWrongRetry ? `
          <button class="quiz-start-btn wrong-retry-btn" id="wrong-retry-btn">
            ❌ 틀린 ${wrongCount}개 다시 복습
          </button>
        ` : ''}
        <button class="quiz-start-btn" id="retry-quiz-btn">🔄 전체 다시 도전</button>
        <a href="#home" class="home-return-btn">🏠 홈으로</a>
      </div>
    </div>
  `;

  document.getElementById('wrong-retry-btn')?.addEventListener('click', () => {
    app.quizDeck = [...app.wrongCards].sort(() => Math.random() - 0.5);
    app.wrongCards = [];
    app.currentCardIndex = 0;
    app.quizScore = 0;
    app.isRetryMode = true;
    renderQuiz(app);
  });
